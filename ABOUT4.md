...
QuestNav (Meta Quest headset) provides robot pose data. At the start of autonomous, `RobotContainer.seedAutoStartPose()` seeds QuestNav with the known starting pose for the selected auto routine, based on the auto name prefix.
...
|------|---------|---------|
| `shoot` | `Score(m_manipulator)` | 3.0 seconds |
...
- **QuestNav seeding** maps the selected auto name prefix to a known starting pose (see `AutoConstants` in `Constants.java`)
